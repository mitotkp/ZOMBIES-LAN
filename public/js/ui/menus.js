...
<div class="mn-root">
  ...
  <section class="mn-screen mn-main" data-screen="main" aria-label="${tr('Pantalla de título')}">
    <div class="mn-main-grid">
      <div class="mn-main-left">
        <div class="mn-title">${titleSVG()}<div class="mn-sub">LAN <span class="mn-dash">—</span> ${esc(tr(MAP_NAME))}</div></div>
        <div class="mn-card mn-join">
          ...
          <div class="mn-actions">
            <button type="button" class="mn-btn primary mn-join-btn">${tr('Unirse a la partida')}</button>
          </div>
  ...
  <section class="mn-screen mn-settings" data-screen="settings" aria-label="${tr('Ajustes')}">
    <div class="mn-panel-box">
      ...
      <div class="st-list">
        <div class="st-row"><label class="st-label" for="zl-st-sens">${tr('Sensibilidad del ratón')}</label>
        ...
        <div class="st-row"><label class="st-label" for="zl-st-fov">${tr('Campo de visión')}</label>
          <input id="zl-st-fov" class="st-range" data-key="fov" type="range" min="60" max="100" step="1"><span class="st-val" data-val="fov"></span></div>
        <div class="st-row"><label class="st-label" for="zl-st-bri">${tr('Brillo')}</label>
        ...
        <div class="st-row"><label class="st-label" for="zl-st-vol">${tr('Volumen general')}</label>
        ...
        <div class="st-row"><label class="st-label" for="zl-st-mus">${tr('Volumen de la música')}</label>
        ...
        <div class="st-row"><span class="st-label">${tr('Calidad gráfica')}</span>
          <div class="st-seg" role="radiogroup" aria-label="${tr('Calidad gráfica')}">
            <button type="button" class="st-seg-btn" data-quality="high">${tr('Alta')}</button>
            <button type="button" class="st-seg-btn" data-quality="low">${tr('Baja')}</button>
          </div><span class="st-val"></span></div>
        <div class="st-row"><label class="st-label" for="zl-st-inv">${tr('Invertir eje Y')}</label>
      ...
      <div class="st-note">${tr('La calidad baja desactiva las sombras y reduce la resolución interna. El suavizado de bordes cambia al recargar la página.')}</div>
      <div class="mn-actions mn-actions-row">
...
function fmtSetting(key, v) {
  switch (key) {
    ...
    case 'invertY': return v ? tr('Sí') : tr('No');
...
export class Menus {
  ...
  _setJoining(on) {
    ...
    this.el.joinBtn.textContent = this.joining ? tr('Conectando…') : tr('Unirse a la partida');
